...
@pytest.fixture
def alt_config(request, web_config):
...
@pytest.fixture
def create_service_meta(container_factory, alt_config):
    ...
    def create(*dependencies, **dependency_map):
        ...
        container.subextensions = set(filter(lambda subext: not isinstance(subext, (ReplyListener, QueueConsumer)),
                                             container.subextensions))
...
@pytest.fixture
def alt_create_service_meta(container_factory, alt_config):
    """ Returns a convenience method for creating service test instance
...
@pytest.fixture
def alt_gateway_service(alt_create_service_meta):
    """ Gateway service test instance with mocked `products_rpc` and
    `orders_rpc` dependencies """
    return alt_create_service_meta('products_rpc', 'orders_rpc')
